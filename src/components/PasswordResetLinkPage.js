...
const PasswordResetLinkPage = () => {
   ...
   const handlePasswordChange = (event) => {
      new SessionStorageUpdates().updatePassword(event.target.value);
    };
  
    const handleConfirmPasswordChange = (event) => {
      new SessionStorageUpdates().updatePasswordConfimation(event.target.value);
    };
  
    const handleConfirmButtonClick = () => {
      new PasswordReset().confirmNewPassword();
    };
  return (
   <div className="desk" style={{ height: '100%' }}>
      <div style={{ display: 'table', width: '100%', height: '100%' }}>
        <div style={{ display: 'table-cell', width: '70%', backgroundColor: 'white', padding: '20px' }}>
          ...
          >
            <div id="password_reset_link_form_content">
              <span style={{ fontSize: '16pt', fontWeight: 300 }}>Pick a new password</span>
              <br />
              <br />
              ...
              <div id="password_reset_link_error" style={{ marginTop: '20px' }}></div>
            </div>
          </div>
        </div>
        <div className="signup_lateral" style={{ display: 'table-cell', width: '30%', padding: '20px', textAlign: 'left' }}>
          ...
          <br />
          <img id="linkedin_logo" src="/static/img/linkedin_logo.png" onClick={() => window.open('https://www.linkedin.com/in/mannanpacha1988')} style={{ height: '20px', cursor: 'pointer' }} alt="LinkedIn Logo" />
